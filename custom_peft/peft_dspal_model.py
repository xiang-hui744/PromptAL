...
class PeftLopaModelForMaskedLM(PeftLopaModel):
	...
	**Attributes**:
	...
	def __init__(self, model, peft_config: PeftConfig, adapter_name="default"):
		...
		self.attention = nn.MultiheadAttention(embed_dim=768, num_heads=ALargs.multi_head,dropout=0.5)
	...
	def forward(
			self,
			latent_prompt_att_weights=None,
			input_ids=None,
			attention_mask=None,
			inputs_embeds=None,
			labels=None,
		...
		if  ALargs.prompt_combine_method == 'self attention':
			x = torch.cat((prompts, latent_prompt_att_weights), dim=1)
			x = x.transpose(0, 1)

			feature, _ = self.attention(x, x, x)
			prompts = feature.transpose(0, 1)

			n = ALargs.v_tokens_n #4
			prompts = prompts[:, :n, :]

		elif ALargs.prompt_combine_method == '按位积':

			# latent_prompt_att_weights
			prompts = prompts * latent_prompt_att_weights



		elif ALargs.prompt_combine_method == '按位加':
			prompts = prompts + latent_prompt_att_weights

		elif ALargs.prompt_combine_method == 'only task':
			prompts = prompts
		elif ALargs.prompt_combine_method == 'only ins':
			prompts = latent_prompt_att_weights
		else:
			raise ValueError(f"Invalid prompt_combine_method: {ALargs.prompt_combine_method}")
		
		# For no sharing [For simply an ablation study]
		# prompts = latent_attention_weights
		#### AL prompts: batch * 10 * 768
		inputs_embeds = torch.cat((prompts, inputs_embeds), dim=1)
		####
		re = self.base_model(inputs_embeds=inputs_embeds,**kwargs)
		# if ALargs.pooling:
		# 	batch_hmask = get_hmask(last_hidden_state=re.hidden_states[-1], mask_indices=attention_mask)
	...
	def xh_get_nb_trainable_parameters(self):
		r"""
		Returns the number of trainable parameters and number of all parameters in the model.
		"""
		trainable_params = 0
	...
	def get_nb_trainable_parameters(self):
		...
		Returns the number of trainable parameters and number of all parameters in the model.
		"""
		trainable_params, all_param = self.xh_get_nb_trainable_parameters()
		return trainable_params, all_param
